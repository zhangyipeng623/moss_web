...
[数据拆分与对比实验技术方案](docs/2026-09-06-design.md) 已形成待评审稿：独立准备数据、训练文件全量参与公共概率与权重校准、固定测试集比较两组系数，并报告公共概率敏感性。[Design Review](docs/2026-09-06-design-review.md) 已通过；用户授权无阻塞问题时直接编写实现计划，尚未修改实现。
...
- [x] 完成 Design Review；依据用户条件授权进入 Plan。
- [x] 编写[实现计划](plans/2026-09-06-implementation.md)，包含五项串行任务、接口与验证命令。
- [ ] 用户批准实现计划后，实施独立准备、公共概率训练与对比实验。
- [ ] 根据实际数据明确时间参数可辨识范围与实验指标。

待办未形成获批实现计划。本轮已完成 Design Review、实现计划及文档结构检查，未运行校准、加载嵌入模型或确认统计指标有效。当前阶段为 Plan；工作流恢复上下文见方案附录。
